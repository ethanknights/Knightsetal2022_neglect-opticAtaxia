function [data,tmpH] = getData_AbsoluteVersion(thisVarStr,rawData,sNames)
...
for s = 1:length(sNames)
  for condition = 1:4
    ...
    for target = 1:7
      [targetStack{s,target,condition},targetMean(s,target,condition),targetStd(s,target,condition)] = getTargetMean_AbsoluteVersion(tmpD,target,idx); %Change!
