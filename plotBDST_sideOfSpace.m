...
for c = 1:4
  ...
  controlMean2(2,c) = mean(controlMean(5:7,c)); %right
  
  controlStd2(1,c) = mean(controlStd(1:3,c)); %left
  controlStd2(2,c) = mean(controlStd(5:7,c)); %right
  
  patientScores2(1,c) = mean(patientScores(1:3,c)); %left
  patientScores2(2,c) = mean(patientScores(5:7,c)); %right  
  
  for controls = 1:length(allControls{c})
    allControls2{c}(controls,1) = mean(allControls{c}(controls,1:3)); %left
    allControls2{c}(controls,2) = mean(allControls{c}(controls,5:7)); %right
  end
